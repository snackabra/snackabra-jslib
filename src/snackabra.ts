...
// we currently have not added proper implementation of IndexedKV / IndexedDB
 ...
 * @param descriptor 
 */
function Online(_target: any, _propertyKey: string, descriptor: PropertyDescriptor) {
  const operation = descriptor.value
  descriptor.value = function (...args: any[]) {
 ...
 */
  ...
  //   // const imageMetaData = controlMessages.find((msg: ChannelMessage) => msg!._id === msgId)!.imageMetaData
  //   console.log(imageMetaData)
  ...
  //   }
  //   console.log("retrieveDataFromMessage: handle is")
  //   console.log(obj)
  //   const img = await this.fetchData(obj)
  //   console.log(img)
...
class ChannelApi {
  ...
  isLocked() {
    return new Promise((resolve, reject) => {
      try {
        SBFetch(this.#channelServer + this.#channel.channelId + '/roomLocked', {
          method: 'GET', credentials: 'include'
        })
          .then((response: Response) => {
            if (!response.ok) {
          ...
          .catch((error: Error) => {
            // console.log("error on accessing (promise)")
            // console.log(error)
            reject(error);
          });
      } catch (e) {
        const eW = WrapError(e)
        console.log("error on accessing")
        console.log(eW)
        reject(eW)
      }
    });
  ...
  create(sbServer: SBServer, serverSecret: string, keys?: JsonWebKey): Promise<SBChannelHandle> {
    return new Promise<SBChannelHandle>(async (resolve, reject) => {
      try {
        ...
        let resp: Dictionary<any> = await SBFetch(sbServer.channel_server + '/api/room/' + channelId + '/uploadRoom', {
          method: 'POST',
          body: data
        });
