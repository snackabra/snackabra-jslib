...
import { SB_libraryVersion, ab2str, str2ab, base64ToArrayBuffer, arrayBufferToBase64, getRandomValues, MessageBus, SBMessage, Snackabra } from './snackabra.js';
...
let channel_id_resolve = (() => { throw new Error('resolve err'); });
let channel_id = new Promise((resolve) => { channel_id_resolve = resolve; });
        ...
        SB.setIdentity(key).then(async () => {
            ...
            c.socket.onMessage = async (message) => {
                ...
                if (controlMessages.length === messages.length * 2 && controlMessages.length > 0) {
                    ...
                    if (messages[0]._id) {
                        let m = messages[0]; // as ChannelMessage
                        // @ts-ignore
